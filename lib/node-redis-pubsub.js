...
NodeRedisPubsub.prototype.on = NodeRedisPubsub.prototype.subscribe = function(channel, handler, callback){
  ...
  var removeListener = function(callback){
    ...
    if (channelRemovers.length === 0)
      return self.receiver.punsubscribe(self.prefix + channel, callback);
  };

  // save the handler and remover so we can use .off() later
  var channelRemovers = this.removers[channel];
  if (!channelRemovers) {
    channelRemovers = [];
    this.removers[channel] = channelRemovers;
  }
  channelRemovers.push({
    handler: handler,
...
NodeRedisPubsub.prototype.off = NodeRedisPubsub.prototype.unsubscribe = function(channel, handler) {
  var channelRemovers = this.removers[channel];
  if (channelRemovers) {
    for (var i in channelRemovers) {
      var data = channelRemovers[i];
      if (data.handler === handler) {
        channelRemovers.splice(i, 1);
        data.remover();
        break;
      }
    }
